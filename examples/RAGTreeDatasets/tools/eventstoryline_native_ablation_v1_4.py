...
"""Native NeoOLAF one-document EventStoryLine experiment support, v1.4.
...
v1.4 focuses on relation recall, direction and evaluation integrity:
...
class EventStoryLineExtractionLayer(v13.EventStoryLineExtractionLayer):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # worker values are ignored because v1.4 constructs the pair pool without
...
""".strip()
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        state.log(
            f"[{self.name}] EventStoryLine v1.4 extraction; "
...
Return JSON only:
{{"decisions":[{{"pair_id":"P00000","decision":"NONE",
...
""".strip()
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        state.log(
            f"[{self.name}] EventStoryLine v1.4 batched five-way enrichment; "
...
def build_pipeline(
    *,
    ...
    pipeline.layers[2] = EventStoryLineCandidateEnrichmentLayer(
        ...
        closure_max_pairs=int(l2_cfg.get("closure_max_pairs", 128)),
    ...
    *,
    ...
    backends = {
        "layer01": _make_backend(
            logger=logger,
            layer_tag="layer01_event_inventory_v1_4",
            model_host=host,
            api_key=api_key,
        ...
        "layer02": _make_backend(
            logger=logger,
            layer_tag="layer02_batched_five_way_relations_v1_4",
            model_host=host,
            api_key=api_key,
