...
def resolve_source_centric_cache_dir() -> Path:
    """Return a deliberately short cache directory, safe for Windows MAX_PATH.

    Override with NEOOLAF_EVENTSTORYLINE_CACHE_DIR. The default lives under the
    operating-system temporary directory instead of the deeply nested run dir.
    """
    raw = os.environ.get("NEOOLAF_EVENTSTORYLINE_CACHE_DIR", "").strip().strip('"').strip("'")
    if raw:
        return Path(raw).expanduser()
    return Path(tempfile.gettempdir()) / "neoolaf_esl16"
...
class EventStoryLineSourceCentricCandidateEnrichmentLayer(v15.EventStoryLineCandidateEnrichmentLayer):
    ...
    def _cache_path(self, messages: list[dict[str, str]], state: v15.PipelineState) -> Path:
        """Use a short deterministic filename to stay far below Windows MAX_PATH."""
        payload = json.dumps(
            {"model": state.llm_model, "messages": messages},
            ensure_ascii=False,
            sort_keys=True,
        )
    ...
    def _chat_custom(
        ...
        """Call the LLM without ever letting cache I/O invalidate a valid response.

        Cache reads and writes are best-effort. Only backend/JSON failures are
        eligible for retry; a Windows path/filesystem error cannot convert a
        successful relation decision into fallback NONE.
        """
        ...
        try:
            ...
            cache_ready = False
            cache_warning = f"cache_mkdir_failed: {type(exc).__name__}: {exc}"
        ...
        if cache_ready:
            try:
                ...
                cache_warning = f"cache_read_failed: {type(exc).__name__}: {exc}"
            except Exception as exc:
                # A corrupt/stale cache must never block a fresh model call.
                cache_warning = f"cache_read_invalid: {type(exc).__name__}: {exc}"
        ...
        for attempt in range(self.retry_failed_calls + 1):
            ...
            # The semantic answer is already valid at this point. Cache persistence
            # is deliberately outside the retry/failure boundary.
            if cache_ready:
                try:
                    with self._cache_lock:
                        write_json(cache_path, parsed)
                except OSError as exc:
                    cache_warning = f"cache_write_failed: {type(exc).__name__}: {exc}"
                except Exception as exc:
                    cache_warning = f"cache_write_failed: {type(exc).__name__}: {exc}"
            ...
            if cache_warning:
                meta["cache_warning"] = cache_warning
            return parsed, meta
...
def build_pipeline(
    *,
    ...
    pipeline.layers[2] = EventStoryLineSourceCentricCandidateEnrichmentLayer(
        ...
        batch_cache_dir=resolve_source_centric_cache_dir(),
