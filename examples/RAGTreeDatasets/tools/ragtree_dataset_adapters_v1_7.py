...
"""Unified4 v1.7 experiment-side adapters.
...
class MavenLayer1FrozenV15(MavenLayer1V14):
    ...
    def _reuse_path(self) -> Path:
        ...
        for marker in ("unified4_v1_7", "unified4_v1_5"):
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        if prior.exists():
            try:
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.7 reused frozen v1.4 source inventory: "
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.7 could not reuse v1.4 inventory; "
...
class MavenLayer2V17(v1._TaskLayer2):
    """MAVEN v1.7 relation-only correction.

    The v1.4 Layer-1 event inventory is frozen and already reaches full gold
    relation-endpoint coverage on the development document. v1.6 showed that the
    remaining bottleneck is *candidate generation*: only 1/14 gold relation pairs
    were ever proposed. v1.7 therefore removes LLM graph-proposal retrieval and
    replaces it with a deterministic, source-only high-recall candidate pool.

    Candidate generation uses only source-grounded mention positions and sentence
    windows. The LLM is used only to adjudicate each directed candidate as
    PRECONDITION / CAUSE / REJECT. No gold is available here.
    """
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        same_sentence_bidirectional = bool(cfg.get("same_sentence_bidirectional", True))
        batch_size = max(8, int(cfg.get("pair_batch_size", 24)))
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_anchor_inventory_v17.json"), anchors)
        ...
        def likely_forward(a: dict[str, Any], b: dict[str, Any]) -> bool:
            return tuple(a["first_pos"]) <= tuple(b["first_pos"])
        ...
        for a in anchors:
            for b in anchors:
                ...
                if dist > sentence_window:
                    continue
                if dist == 0 and not same_sentence_bidirectional and not likely_forward(a, b):
                    continue
                if dist > 0 and not likely_forward(a, b):
                    continue
                cue_count, cue_sids, cue_reasons = cue_score(a, b)
                same_sentence = dist == 0
                hint = "PRECONDITION"
                if same_sentence and cue_count > 0:
                    hint = "CAUSE"
                reason_parts = [f"deterministic sentence-window candidate (distance={dist})"] + cue_reasons
                raw_candidates.append({
                    "source": a["anchor_id"],
                    "target": b["anchor_id"],
                    "proposal": "deterministic_sentence_window",
                    "label_hint": hint,
        ...
        raw_candidates.sort(key=lambda e: (
            ...
            0 if likely_forward(anchor_by_id[e["source"]], anchor_by_id[e["target"]]) else 1,
            e["source"], e["target"],
        ))
        ...
        for i, edge in enumerate(raw_candidates):
            directed.append({
                **edge,
                ...
                "label_hints": [edge["label_hint"]] if edge.get("label_hint") else [],
                "supports": [edge["support"]] if edge.get("support") else [],
                "proposal_count": 1,
                "independent_graph_proposal_count": 1,
            })
        v1.write_json(self.decision_log_path.with_name("layer02_maven_directed_candidates_v17.json"), directed)
...
KEEP_PRECONDITION, KEEP_CAUSE, REVERSE_PRECONDITION, REVERSE_CAUSE, or REJECT.
...
""".strip()},
            ...
            parsed = self._chat(state, messages, f"deterministic_candidate_adjudicate_v17_{idx:03d}")
        ...
        for pid, edge in by_pid.items():
            ...
            positive = decision in {"KEEP_PRECONDITION", "KEEP_CAUSE", "REVERSE_PRECONDITION", "REVERSE_CAUSE"}
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_decisions_v17.json"), decision_audit)
        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_survivors_v17.json"), accepted)
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_final_edges_v17.json"), final_edges)

        for i, edge in enumerate(final_edges):
            enriched.append(v1._relation_enriched(
                expr_id=f"expr_mr17_{i:04d}", source=edge["source_key"], relation_id=edge["label"], target=edge["target_key"],
                metadata=self.catalog[edge["label"]], state=state,
                decision_payload={**edge, "decision": edge["label"], "pipeline": "v1.7_deterministic_candidate_pool"},
            ))
        ...
        counts = {
            ...
            "candidate_generation": "deterministic_sentence_window",
            "sentence_window": sentence_window,
            "same_sentence_bidirectional": same_sentence_bidirectional,
            "gold_used": False,
        }
        v1.write_json(self.decision_log_path.with_name("layer02_maven_stage_counts_v17.json"), counts)
        return self._finish(state, enriched)
...
def _maven_stage_posthoc_v16(gold_record: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    files = {
        "directed_candidates": ("layer02_maven_directed_candidates_v17.json", False),
        "verifier_survivors": ("layer02_maven_verifier_survivors_v17.json", True),
        "final_edges": ("layer02_maven_final_edges_v17.json", True),
    }
    stages: dict[str, Any] = {}
    ...
    final_path = run_dir / "run_logs" / "layer02_maven_final_edges_v17.json"
    ...
    *, dataset_key: str, backends: dict[str, Any], rag_adapter: Any,
    ...
    if dataset_key == "fincausal":
        ...
        pipeline.layers[2] = FinCausalLayer2V13(
            ...
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v17.json",
        ...
        pipeline.layers[2] = MavenLayer2V17(
            backends["layer02"], dataset_key=dataset_key,
            relation_catalog_path=relation_catalog_path,
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v17.json",
...
def run_native_pipeline_record(
    *, dataset_key: str, project_root: str | Path, input_jsonl: str | Path, ontology_path: str | Path,
    ...
    backends = {
        "layer01": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer01_v17", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer01_linguistic_expression_extraction"), fallback_max_tokens=max_tokens, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer02": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer02_v17", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer02_candidate_enrichment"), fallback_max_tokens=4096, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer04": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer04_v17", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer04_candidate_relation_extraction"), fallback_max_tokens=384, fallback_timeout=60, reasoning_effort=reasoning_effort),
        "other": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_other_v17", model_host=host, api_key=api_key, cfg={}, fallback_max_tokens=768, fallback_timeout=90, reasoning_effort=reasoning_effort),
    }
...
def evaluate_state(dataset_key: str, state: PipelineState, gold_record: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        stage_path = run_dir / "run_logs" / "layer02_maven_stage_counts_v17.json"
