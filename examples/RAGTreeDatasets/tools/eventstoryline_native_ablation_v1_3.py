...
"""Native NeoOLAF one-document EventStoryLine experiment support, v1.3.
...
profile-driven three-stage orchestration for mention-level event extraction and the two
...
class EventStoryLineExtractionLayer(v4.LinguisticExpressionExtractionLayer):
    """Recall-oriented Layer 1 with validated mention identity.

    Stage A1 runs independent sentence-level event inventory calls in parallel.
    Stage A2 performs whole-document coverage reviews focused on missed eventive
    nominals, states, repeated mentions, and embedded predicates. Every proposal
    is validated against authoritative source tokens and may receive only bounded
    source-token span repair. Stage B generates storyline pairs in parallel source
    batches over the resulting closed event inventory.

    Gold event IDs, gold spans, and gold pairs are unavailable in all stages.
    """
    ...
    # Conservative fallback only. Complete trigger text is aligned first; this
    # list prevents nouns such as "stay" or "violation" from being expanded to
    # a following preposition merely because it is adjacent in the sentence.
    _PHRASAL_HEADS = {
        "break", "broke", "broken", "bring", "brought", "carry", "carried",
        "check", "checks", "checked", "checking", "come", "came", "coming", "get", "got",
        "getting", "go", "goes", "going", "went", "look", "looked", "looking", "move",
        "moved", "moving", "pick", "picked", "pull", "pulls", "pulled", "pulling", "put",
        "run", "ran", "running", "set", "setting", "show", "shows", "showed", "showing",
        "take", "took", "taking", "turn", "turned", "turning",
    }
    _DATE_METADATA_WORDS = {
        "am", "pm", "edt", "est", "utc", "published", "updated", "originally",
    ...
    def __init__(
        ...
        *args: Any,
        ...
        call_audit_path: str | Path | None = None,
        sentence_workers: int = 8,
        coverage_review_passes: int = 2,
        relation_workers: int = 8,
        relation_source_batch_size: int = 4,
        **kwargs: Any,
    ) -> None:
        ...
        self.call_audit_path = Path(call_audit_path or base / "layer01_call_audit.json")
        self.sentence_workers = max(1, int(sentence_workers))
        self.coverage_review_passes = max(0, int(coverage_review_passes))
        self.relation_workers = max(1, int(relation_workers))
        self.relation_source_batch_size = max(1, int(relation_source_batch_size))
    ...
    def _extend_source_span(
        ...
        if repaired_end < len(sentence_tokens):
            ...
            current_head = _norm(current_values[-1]) if current_values else ""
            if (
                valid_current
                and current_head in cls._PHRASAL_HEADS
                and next_norm in cls._PHRASAL_PARTICLES
                and repaired_end - start <= 3
            ):
                repaired_end += 1
    ...
    @classmethod
    def _sentence_is_non_story_metadata(cls, sentence: str, sentence_tokens: list[str]) -> tuple[bool, str]:
        norms = [_norm(token) for token in sentence_tokens]
        norms = [value for value in norms if value]
        lowered = str(sentence or "").lower()
        if norms and norms[0] in {"http", "https"}:
            return True, "url_sentence"
        if "http" in lowered and ("/" in sentence or ".com" in lowered):
            return True, "url_sentence"
        if not norms:
            return True, "empty_sentence"
        lexical_non_metadata = [
            value for value in norms
            if value not in cls._DATE_METADATA_WORDS and not value.isdigit()
        ]
        if len(lexical_non_metadata) == 0:
            return True, "date_or_publication_metadata"
        return False, "story_sentence"

    @staticmethod
    def _event_items(parsed: Any) -> list[dict[str, Any]]:
        if not isinstance(parsed, dict):
            return []
        raw_items = parsed.get("event_mentions")
        if raw_items is None:
            raw_items = parsed.get("missing_event_mentions")
        if raw_items is None:
            raw_items = [
                item for item in (parsed.get("expressions") or [])
    ...
    def _sentence_event_prompt(
        ...
        indexed = " ".join(f"{index}={token}" for index, token in enumerate(sentence_tokens))
        system = """
You are NeoOLAF Layer 1A1 for high-recall EventStoryLine event-mention extraction.

Inspect exactly one story sentence. Extract EVERY explicit mention-level event,
action, process, occurrence, change, or state that could serve as an endpoint of
a storyline/plot relation. EventStoryLine event triggers are broader than only
main verbs. Include:
- lexical verbs, participles, infinitival events, and complete phrasal verbs;
- eventive nouns and nominalizations, including legal, administrative, social,
  medical, movement, communication, and process events;
- explicit states or conditions evoked by adjectives, nouns, or multi-token phrases;
- embedded/subordinate events and each repeated mention as a separate indexed mention.

Use zero-based, end-exclusive offsets from the authoritative token table. The
trigger must be the smallest COMPLETE semantic trigger. Preserve particles and
hyphen pieces. Do not output people, organizations, places, dates, objects,
connectives alone, auxiliaries alone, or inferred events absent from the sentence.

Return JSON only:
{"event_mentions":[{"sentence_id":3,"token_start":4,"token_end":5,
"trigger":"entering","event_kind":"process","justification":"Explicit event."}],
"coverage_check":{"sentence_id":3,"checked":true}}
""".strip()
        user = f"""
Dataset guidance:
...
Return every explicit event mention in this sentence. JSON only.
""".strip()
    ...
    def _coverage_review_prompt(
        ...
        focus = (
            "all missing explicit event mentions, including embedded predicates and repeated mentions"
            if pass_index == 1
            else "missed eventive nouns/nominalizations, legal or administrative processes, explicit states/conditions, participles, and repeated mentions with different spans"
        )
        existing = [
...
The first extraction pass has already produced a validated event inventory. Find
ONLY missing explicit event mentions. This review pass focuses on {focus}.

EventStoryLine treats mention-level event triggers broadly: verbs, eventive nouns,
nominalizations, occurrences, processes, actions, and explicit states can all be
events. Preserve repeated mentions when their sentence/token spans differ. Do not
repeat an existing event, create relations, infer absent events, or output entities,
dates, connectives, or auxiliaries alone. Use exact zero-based end-exclusive spans
from the authoritative token table.

Return JSON only:
{{"missing_event_mentions":[{{"sentence_id":4,"token_start":11,"token_end":13,
"trigger":"court hearing","event_kind":"eventive_nominal","justification":"Missing explicit event."}}],
"coverage_check":{{"pass":{pass_index},"checked":true}}}}
""".strip()
        user = f"""
Dataset guidance:
{_json_block((task.get('layer_guidance') or {}).get('layer01', {}), 5500)}

Already validated events (do not repeat):
{_json_block(existing, 12000)}
...
""".strip()
    ...
    def _relation_batch_prompt(
        ...
        task = (state.profile_config or {}).get("_input_task_guidance", {}) or {}
        inventory = [
            {"event_id": row["event_ref"], "event_key": row["event_key"], "sentence": row.get("sentence", "")}
            for row in event_rows
        ]
        sources = [row["event_ref"] for row in source_rows]
        system = """
You are NeoOLAF Layer 1B for high-recall EventStoryLine PLOT_LINK discovery.

For each assigned source event, identify every supported directed storyline link
to a target in the CLOSED validated inventory. EventStoryLine plot links are
broader than strict physical causality. A supported link may express setup,
prerequisite, enabling context, motivation, consequence, continuation, aftermath,
follow-up, elaboration, or a headline/body narrative development. Cross-sentence
links and links between repeated mentions are allowed when the text establishes a
real plot dependency. Mere co-occurrence or sentence order is insufficient.

Rules:
- source_event_id must be one of ASSIGNED_SOURCE_IDS;
- target_event_id must come from the supplied inventory;
- never create/alter spans, swap direction, emit self-links, or emit null;
- relation_cue must be a short lexical/semantic cue, not PRECONDITION or FALLING_ACTION;
- include all plausible supported targets for every assigned source, then perform a
  source-by-source coverage check. Layer 2 will make the final two-class decision.

Return JSON only:
{"relation_instances":[{"source_event_id":"E0001","relation_cue":"sets up",
"target_event_id":"E0004","justification":"Document-supported plot dependency."}],
"coverage_check":{"batch":0,"checked_source_ids":["E0001"],"checked":true}}
""".strip()
        user = f"""
ASSIGNED_SOURCE_IDS: {json.dumps(sources)}
Relation-family definitions:
{_json_block(task.get('relation_specs') or [], 7000)}
Synthetic examples:
{_json_block(task.get('synthetic_relation_examples') or [], 5500)}
Closed event inventory:
{_json_block(inventory, 18000)}
Raw document:
\"\"\"
{chunk_text}
\"\"\"
Return all supported pairs for assigned sources only. JSON only.
""".strip()
        return [{"role":"system","content":system},{"role":"user","content":user}]

    def _parse_chat(self, messages: list[dict[str, str]], state: PipelineState) -> dict[str, Any]:
        raw = self.ollama_backend.chat(model=state.llm_model, messages=messages, temperature=self.temperature)
        parsed = self.ollama_backend.extract_json(raw)
        if not isinstance(parsed, dict):
            raise ValueError(f"Layer 1 response must be a JSON object, got {type(parsed).__name__}")
        return parsed

    def _ingest_events(
        self,
        *, items: list[dict[str, Any]], phase: str, tokens: list[list[str]],
        sentences: list[str], accepted_by_key: dict[str, dict[str, Any]],
        inventory_audit: list[dict[str, Any]], combined_decisions: list[dict[str, Any]],
        expected_sent_id: int | None = None,
    ) -> int:
        accepted_count = 0
        for raw_item in items:
            item = dict(raw_item)
            proposed_sent = self._item_int(item, "sentence_id", "sent_id")
            if expected_sent_id is not None:
                if proposed_sent is None:
                    item["sentence_id"] = expected_sent_id
                elif proposed_sent != expected_sent_id:
                    audit = {
                        "phase": phase, "raw_item": raw_item, "status": "rejected",
                        "reason": "sentence_call_cross_sentence_id",
                        "expected_sentence_id": expected_sent_id,
                        "proposed_sentence_id": proposed_sent,
                    }
                    inventory_audit.append(audit)
                    combined_decisions.append({**audit, "label":"event_mention"})
                    continue
            ...
            if canonical is None:
                combined_decisions.append({
                    "phase": phase, "status":"rejected", "reason":audit.get("reason"),
                    "text":item.get("event_key") or item.get("text") or item.get("trigger"),
                    "label":"event_mention", "repair_steps":audit.get("repair_steps", []),
                })
                continue
            parsed = parse_event_key(canonical) or {}
            sent_id = int(parsed.get("sent_id", -1))
            existing = canonical in accepted_by_key
            ...
            if existing:
                accepted_by_key[canonical].setdefault("source_phases", []).append(phase)
            else:
                accepted_count += 1
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        for chunk in chunks:
            ...
            # Stage A1: independent sentence calls, bounded in memory and parallel.
            ...
            for sent_id in sorted(story_sentence_ids):
                ...
                added = self._ingest_events(
                    items=items, phase="sentence_inventory", tokens=tokens, sentences=sentences,
                    accepted_by_key=accepted_by_key, inventory_audit=inventory_audit,
                    combined_decisions=combined_decisions, expected_sent_id=sent_id,
                )
                call_audit.append({
                    "phase":"sentence_inventory","sentence_id":sent_id,"status":"ok",
                    "proposals":len(items),"new_validated_events":added,
                })

            # Stage A2: sequential whole-document reviews; each sees the current union.
            for pass_index in range(1, self.coverage_review_passes + 1):
                current_rows = [accepted_by_key[key] for key in sorted(accepted_by_key)]
                messages = self._coverage_review_prompt(state, chunk.text, current_rows, pass_index)
                try:
                    parsed = self._parse_chat(messages, state)
                    items = self._event_items(parsed)
                    added = self._ingest_events(
                        items=items, phase=f"coverage_review_{pass_index}", tokens=tokens, sentences=sentences,
                        accepted_by_key=accepted_by_key, inventory_audit=inventory_audit,
                        combined_decisions=combined_decisions,
                    )
                    call_audit.append({
                        "phase":"coverage_review","pass_index":pass_index,"status":"ok",
                        "proposals":len(items),"new_validated_events":added,
                        "inventory_size_after":len(accepted_by_key),
                    })
                except Exception as exc:
                    call_audit.append({
                        "phase":"coverage_review","pass_index":pass_index,"status":"error",
                        "error":f"{type(exc).__name__}: {exc}",
                    })

            event_rows: list[dict[str, Any]] = []
            sort_key = lambda value: (
                (parse_event_key(value) or {}).get("sent_id", 10**9),
                (parse_event_key(value) or {}).get("start", 10**9),
                (parse_event_key(value) or {}).get("end", 10**9),
            )
            for index, key in enumerate(sorted(accepted_by_key, key=sort_key)):
                row = dict(accepted_by_key[key])
                row["event_ref"] = f"E{index:04d}"
                row["source_phases"] = _dedup(row.get("source_phases", []))
                event_rows.append(row)

            # Stage B: complete source coverage in parallel bounded batches.
            relation_items_with_batch: list[tuple[int, set[str], dict[str, Any]]] = []
            source_batches = self._batches(event_rows, self.relation_source_batch_size)
            if source_batches:
                with ThreadPoolExecutor(max_workers=min(self.relation_workers, len(source_batches))) as executor:
                    futures = {}
                    for batch_index, source_rows in enumerate(source_batches):
                        source_ids = {row["event_ref"] for row in source_rows}
                        messages = self._relation_batch_prompt(
                            state, chunk.text, event_rows, source_rows, batch_index
                        )
                        futures[executor.submit(self._parse_chat, messages, state)] = (batch_index, source_ids)
                    for future in as_completed(futures):
                        batch_index, source_ids = futures[future]
                        try:
                            parsed = future.result()
                            items = [
                                item for item in (parsed.get("relation_instances") or [])
                                if isinstance(item, dict)
                            ]
                            for item in items:
                                relation_items_with_batch.append((batch_index, source_ids, item))
                            call_audit.append({
                                "phase":"relation_source_batch","batch_index":batch_index,"status":"ok",
                                "assigned_source_ids":sorted(source_ids),"proposals":len(items),
                            })
                        except Exception as exc:
                            call_audit.append({
                                "phase":"relation_source_batch","batch_index":batch_index,"status":"error",
                                "assigned_source_ids":sorted(source_ids),
                                "error":f"{type(exc).__name__}: {exc}",
                            })
            ...
            for batch_index, assigned_source_ids, item in sorted(
                relation_items_with_batch,
                key=lambda value: (value[0], str(value[2].get("source_event_id", "")), str(value[2].get("target_event_id", ""))),
            ):
                ...
                audit = {
                    "phase":"relation_source_batch","batch_index":batch_index,
                    "assigned_source_ids":sorted(assigned_source_ids),
                    "source_event_id":source_ref,"target_event_id":target_ref,
                    "relation_cue":cue,"justification":justification,
                }
                if source_ref not in assigned_source_ids:
                    audit.update(status="rejected", reason="source_not_assigned_to_batch")
                elif source_ref not in by_ref or target_ref not in by_ref:
                ...
                elif not cue or normalize_relation_id(cue) is not None:
                    ...
                    if identity in relation_seen:
                        ...
                        relation_text = f"{source_key} || {cue} || {target_key}"
                        relation_rows.append({"text":relation_text,"label":"relation_instance","justification":justification})
                        audit.update(status="accepted", reason="closed_inventory_source_batch_relation", relation_instance=relation_text)
                relation_audit.append(audit)
                if audit.get("status") == "rejected":
                    combined_decisions.append({**audit,"label":"relation_instance"})

            accepted_rows: list[dict[str, str]] = [
                {"text":row["event_key"],"label":"event_mention","justification":row["justification"]}
                for row in event_rows
            ]
            ...
            for row in accepted_rows:
                ...
                expressions.append(LinguisticExpression(
                    ...
                    evidence=[Evidence(
                        chunk_id=chunk.chunk_id, chunk_start_char=-1, chunk_end_char=-1,
                        doc_start_char=-1, doc_end_char=-1, snippet=chunk.text[:1600],
                    )],
                ))
                combined_decisions.append({
                    "phase":"materialization","status":"accepted","expr_id":expr_id,
                    "text":text_value,"label":label,
                    "relation_instance":_parse_relation_instance(text_value),
                    "justification":justification,
                })
                expr_counter += 1
        ...
        state.log(
            f"[{self.name}] EventStoryLine recall-oriented extraction; "
            ...
            f"sentence_calls={sum(1 for x in call_audit if x.get('phase') == 'sentence_inventory' and x.get('status') == 'ok')}; "
            f"coverage_reviews={sum(1 for x in call_audit if x.get('phase') == 'coverage_review' and x.get('status') == 'ok')}; "
            f"relation_batches={sum(1 for x in call_audit if x.get('phase') == 'relation_source_batch' and x.get('status') == 'ok')}"
        )
        return state
...
EventStoryLine PLOT_LINK is broader than strict physical causality.
PRECONDITION: the source is rising-action context that establishes, enables,
motivates, requires, prepares, or sets up the target.
FALLING_ACTION: the target is a downstream consequence, continuation, aftermath,
follow-up, elaboration, or headline/body narrative development connected from the source.

Cross-sentence and repeated-mention links are permitted when the document supports
a real narrative dependency. Sentence order alone and topical co-occurrence are insufficient. Return JSON only:
...
def build_pipeline(
    *, backends: dict[str, TaggedLoggedBackend], rag_adapter: Any,
    ...
    pipeline.layers[1] = EventStoryLineExtractionLayer(
        ...
        coverage_review_passes=int(l1_cfg.get("coverage_review_passes", 2)),
        relation_workers=int(l1_cfg.get("relation_workers", min(workers, 8))),
        relation_source_batch_size=int(l1_cfg.get("relation_source_batch_size", 4)),
        max_chunks=1,
        temperature=0.0,
...
def run_native_pipeline(
    *, project_root: str | Path, input_jsonl: str | Path, ontology_path: str | Path,
    ...
    manifest = {
        ...
        "layer01_three_stage_closed_inventory":True,
        "layer01_bounded_span_repair":True,
        "layer01_sentence_parallelism":int(_layer_cfg(profile_dict, "layer01_linguistic_expression_extraction").get("sentence_workers", min(workers, 8))),
        "layer01_coverage_review_passes":int(_layer_cfg(profile_dict, "layer01_linguistic_expression_extraction").get("coverage_review_passes", 2)),
        "layer01_relation_source_batch_size":int(_layer_cfg(profile_dict, "layer01_linguistic_expression_extraction").get("relation_source_batch_size", 4)),
