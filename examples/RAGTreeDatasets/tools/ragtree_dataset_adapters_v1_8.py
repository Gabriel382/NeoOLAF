...
"""Unified4 v1.8 experiment-side adapters.
...
class MavenLayer1FrozenV15(MavenLayer1V14):
    ...
    def _reuse_path(self) -> Path:
        ...
        for marker in ("unified4_v1_8", "unified4_v1_7", "unified4_v1_5"):
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        if prior.exists():
            try:
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.8 reused frozen v1.4 source inventory: "
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.8 could not reuse v1.4 inventory; "
...
class MavenLayer2V18(v1._TaskLayer2):
    """MAVEN v1.8 relation-only correction.
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        # v1.8: every ordered pair A->B and B->A within the local sentence window
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_anchor_inventory_v18.json"), anchors)
        ...
        for a in anchors:
            for b in anchors:
                ...
                raw_candidates.append({
                    "source": a["anchor_id"],
                    "target": b["anchor_id"],
                    "proposal": "deterministic_all_ordered_local_pairs_v18",
                    "label_hint": "",
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_directed_candidates_v18.json"), directed)
...
""".strip()},
            ...
            parsed = self._chat(state, messages, f"deterministic_candidate_adjudicate_v18_{idx:03d}")
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_decisions_v18.json"), decision_audit)
        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_survivors_v18.json"), accepted)
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_final_edges_v18.json"), final_edges)

        for i, edge in enumerate(final_edges):
            enriched.append(v1._relation_enriched(
                expr_id=f"expr_mr18_{i:04d}", source=edge["source_key"], relation_id=edge["label"], target=edge["target_key"],
                metadata=self.catalog[edge["label"]], state=state,
                decision_payload={**edge, "decision": edge["label"], "pipeline": "v1.8_all_ordered_local_pairs"},
            ))
        ...
        counts = {
            ...
            "candidate_generation": "deterministic_all_ordered_local_pairs_v18",
            "sentence_window": sentence_window,
            "preclassification_candidate_cap": None,
            "semantic_direction_filter_before_llm": False,
            "gold_used": False,
        }
        v1.write_json(self.decision_log_path.with_name("layer02_maven_stage_counts_v18.json"), counts)
        return self._finish(state, enriched)
...
def _maven_stage_posthoc_v16(gold_record: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    files = {
        "directed_candidates": ("layer02_maven_directed_candidates_v18.json", False),
        "verifier_survivors": ("layer02_maven_verifier_survivors_v18.json", True),
        "final_edges": ("layer02_maven_final_edges_v18.json", True),
    }
    stages: dict[str, Any] = {}
    ...
    final_path = run_dir / "run_logs" / "layer02_maven_final_edges_v18.json"
    ...
    *, dataset_key: str, backends: dict[str, Any], rag_adapter: Any,
    ...
    if dataset_key == "fincausal":
        ...
        pipeline.layers[2] = FinCausalLayer2V13(
            ...
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v18.json",
        ...
        pipeline.layers[2] = MavenLayer2V18(
            backends["layer02"], dataset_key=dataset_key,
            relation_catalog_path=relation_catalog_path,
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v18.json",
...
def run_native_pipeline_record(
    *, dataset_key: str, project_root: str | Path, input_jsonl: str | Path, ontology_path: str | Path,
    ...
    backends = {
        "layer01": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer01_v18", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer01_linguistic_expression_extraction"), fallback_max_tokens=max_tokens, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer02": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer02_v18", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer02_candidate_enrichment"), fallback_max_tokens=4096, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer04": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer04_v18", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer04_candidate_relation_extraction"), fallback_max_tokens=384, fallback_timeout=60, reasoning_effort=reasoning_effort),
        "other": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_other_v18", model_host=host, api_key=api_key, cfg={}, fallback_max_tokens=768, fallback_timeout=90, reasoning_effort=reasoning_effort),
    }
...
def evaluate_state(dataset_key: str, state: PipelineState, gold_record: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        stage_path = run_dir / "run_logs" / "layer02_maven_stage_counts_v18.json"
