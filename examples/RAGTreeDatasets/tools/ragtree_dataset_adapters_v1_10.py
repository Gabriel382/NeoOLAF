...
"""Unified4 v1.10 experiment-side adapters.
...
* MAVEN-ERE: v1.10 returns to the v1.8 high-recall relation architecture, then applies one conservative document-level false-positive pruning pass without adding, relabeling, or reversing relations.
...
class MavenLayer1FrozenV15(MavenLayer1V14):
    ...
    v1.10 is relation-only above the frozen event inventory. This layer first reconstructs the exact source-grounded
    ...
    def _reuse_path(self) -> Path:
        ...
        for marker in ("unified4_v1_10", "unified4_v1_8", "unified4_v1_7", "unified4_v1_5"):
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        if prior.exists():
            try:
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.10 reused frozen v1.4 source inventory: "
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.10 could not reuse v1.4 inventory; "
...
class MavenLayer2V110(v1._TaskLayer2):
    """MAVEN v1.10: v1.8 high-recall relation generation + conservative graph pruning.

    Layers 1 and the pairwise relation adjudicator are intentionally the v1.8 design:
    all ordered source-grounded event pairs within sentence distance <= 4 are exposed
    to the classifier, with no pre-classification direction filter or candidate cap.

    v1.10 adds exactly one downstream precision step before native Layer 3: a
    conservative document-level review may REMOVE only relations that are clearly
    unsupported (chronology/topic/coreference/no dependency). It cannot add, relabel,
    reverse, or replace an edge. Missing/uncertain review decisions default to KEEP.
    Gold remains unavailable to every pipeline layer.
    """
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        # v1.10: every ordered pair A->B and B->A within the local sentence window
        # is retained. There is no textual-direction filter and no pre-classification cap.
        # Semantic direction is decided only by the relation classifier.
        multi_min = float(cfg.get("multi_proposal_min_confidence", 0.50))
        single_min = float(cfg.get("single_proposal_min_confidence", 0.58))
        max_final = max(16, int(cfg.get("max_final_relations", 192)))
        ...
        batch_size = max(8, int(cfg.get("pair_batch_size", 48)))
        graph_review_enabled = bool(cfg.get("graph_review_enabled", True))
        graph_review_batch_size = max(16, int(cfg.get("graph_review_batch_size", 48)))
        graph_review_remove_min_confidence = float(cfg.get("graph_review_remove_min_confidence", 0.86))
        graph_review_max_remove_fraction = min(0.60, max(0.0, float(cfg.get("graph_review_max_remove_fraction", 0.40))))
        rows_doc = v1._safe_token_rows(state)
        sentence_text = {int(r["sentence_id"]): str(r.get("sentence") or "") for r in rows_doc}

        anchors: list[dict[str, Any]] = []
        for i, key in enumerate(keys):
            positions = v1._maven_cluster_positions(key)
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_anchor_inventory_v110.json"), anchors)
        ...
        def cue_score(a: dict[str, Any], b: dict[str, Any]) -> tuple[int, list[int], list[str]]:
            sids = sorted(set(a["sentence_ids"]) | set(b["sentence_ids"]))
            lo, hi = min(sids), max(sids)
            scope = list(range(lo, hi + 1))
            hits = [sid for sid in scope if _MAVEN_CAUSAL_CUE_RE.search(sentence_text.get(sid, ""))]
            reasons: list[str] = []
            if hits:
                reasons.append("source window contains causal/purpose/temporal dependency cue")
            return len(hits), hits, reasons
        ...
        for a in anchors:
            for b in anchors:
                if a["anchor_id"] == b["anchor_id"]:
                    continue
                dist = min_sentence_distance(a, b)
                if dist > sentence_window:
                    continue
                cue_count, cue_sids, cue_reasons = cue_score(a, b)
                reason_parts = [f"deterministic ordered local candidate (distance={dist})"] + cue_reasons
                raw_candidates.append({
                    "source": a["anchor_id"],
                    "target": b["anchor_id"],
                    "proposal": "deterministic_all_ordered_local_pairs_v110",
                    "label_hint": "",
                    "support": "; ".join(reason_parts),
                    "evidence_sentence_ids": cue_sids,
                    "min_sentence_distance": dist,
                    "cue_count": cue_count,
                    "same_sentence": dist == 0,
                    "source_key": a["key"],
                    "target_key": b["key"],
                    "source_trigger": a["trigger"],
                    "target_trigger": b["trigger"],
                    "source_positions": a["positions"],
                    "target_positions": b["positions"],
                })

        # Sorting is only for reproducible batching. No candidate is removed.
        raw_candidates.sort(key=lambda e: (
            e["min_sentence_distance"],
            -int(e["cue_count"] > 0),
            e["source"], e["target"],
        ))

        directed: list[dict[str, Any]] = []
        for i, edge in enumerate(raw_candidates):
            directed.append({
                **edge,
                "pair_id": f"D{i:04d}",
                "proposals": [edge["proposal"]],
                "label_hints": [],
                "supports": [edge["support"]] if edge.get("support") else [],
                "proposal_count": 1,
                "independent_graph_proposal_count": 1,
            })
        v1.write_json(self.decision_log_path.with_name("layer02_maven_directed_candidates_v110.json"), directed)

        decisions: dict[str, dict[str, Any]] = {}
        batches = [directed[i:i + batch_size] for i in range(0, len(directed), batch_size)]

        def run_adjudicate(idx: int, batch: list[dict[str, Any]]) -> list[dict[str, Any]]:
            compact = [{
                "pair_id": e["pair_id"],
                "source_trigger": e["source_trigger"],
                "target_trigger": e["target_trigger"],
                "source_positions": e["source_positions"],
                "target_positions": e["target_positions"],
                "source_first_pos": anchor_by_id[e["source"]]["first_pos"],
                "target_first_pos": anchor_by_id[e["target"]]["first_pos"],
                "min_sentence_distance": e["min_sentence_distance"],
                "candidate_origin": e["proposal"],
                "label_hint": e["label_hint"],
                "heuristic_support": e["support"],
                "evidence_sentence_ids": e["evidence_sentence_ids"],
            } for e in batch]
            messages = [
                {"role": "system", "content": """
You are NeoOLAF MAVEN-ERE directed relation adjudication. Every supplied pair is an EXISTENCE candidate generated deterministically from source-local event mentions. Your job is ONLY to classify each pair as:
KEEP_PRECONDITION, KEEP_CAUSE, or REJECT.

Use the MAVEN relation convention:
- PRECONDITION: TARGET depends on SOURCE as a prerequisite, enabling, initiating, supporting, planning, structural, or process condition. Strongly implied narrative/event-structure dependency is enough.
- CAUSE: SOURCE more directly brings about, produces, triggers, or makes TARGET effectively follow.
- REJECT chronology-only, same-topic association, lexical similarity, coreference, and speculative unsupported links.

Important calibration:
- The candidate pool is high-recall and source-local. Many pairs are false; REJECT freely.
- However, do NOT require explicit counterfactual wording for PRECONDITION.
- One SOURCE may validly support multiple TARGET events.
- Direction is semantic, never textual mention order.

Confidence: 0.50-0.65 plausible, 0.66-0.80 clear, >0.80 very clear.
Return support_tier DIRECT, STRONG_IMPLICIT, WEAK, or NONE. Positives may use DIRECT or STRONG_IMPLICIT.
JSON only:
{"decisions":[{"pair_id":"D0000","decision":"KEEP_PRECONDITION|KEEP_CAUSE|REJECT","support_tier":"DIRECT|STRONG_IMPLICIT|WEAK|NONE","evidence_sentence_ids":[1,5],"confidence":0.67,"reason":"..."}]}
""".strip()},
                {"role": "user", "content": (
                    f"Document:\n{v1._json_block(rows_doc, 52000)}\n\n"
                    f"Directed candidates:\n{v1._json_block(compact, 42000)}"
                )},
            ]
            parsed = self._chat(state, messages, f"deterministic_candidate_adjudicate_v110_{idx:03d}")
            rows = parsed.get("decisions", []) if isinstance(parsed, dict) else []
            return rows if isinstance(rows, list) else []

        if batches:
            with ThreadPoolExecutor(max_workers=min(self.max_concurrency, len(batches))) as ex:
                futures = {ex.submit(run_adjudicate, i, batch): i for i, batch in enumerate(batches)}
                for fut in as_completed(futures):
                    try:
                        rows = fut.result()
                    except Exception as exc:
                        self._record({"phase": "adjudicate", "batch": futures[fut], "status": "error", "error": str(exc)})
                        continue
                    for row in rows:
                        if isinstance(row, dict) and row.get("pair_id"):
                            decisions[str(row["pair_id"])] = row
        ...
        by_pid = {e["pair_id"]: e for e in directed}
        for pid, edge in by_pid.items():
            row = decisions.get(pid, {
                "decision": "REJECT", "support_tier": "NONE", "confidence": 0.0,
                "reason": "missing adjudication decision",
            })
            decision = str(row.get("decision") or "REJECT").upper()
            support = str(row.get("support_tier") or "NONE").upper()
            try:
                confidence = float(row.get("confidence", 0.0) or 0.0)
            except Exception:
                confidence = 0.0
            graph_count = int(edge.get("independent_graph_proposal_count", 1) or 1)
            threshold = multi_min if graph_count >= 2 else single_min
            positive = decision in {"KEEP_PRECONDITION", "KEEP_CAUSE"}
            accept = positive and support in {"DIRECT", "STRONG_IMPLICIT"} and confidence >= threshold
            audit = {**edge, **row, "acceptance_threshold": threshold, "accepted_by_threshold": accept}
            decision_audit.append(audit)
            self._record({"phase": "deterministic_candidate_adjudicate", **audit})
            if not accept:
                continue
            source, target = edge["source"], edge["target"]
            source_key, target_key = edge["source_key"], edge["target_key"]
            label = "PRECONDITION" if "PRECONDITION" in decision else "CAUSE"
            accepted.append({
                "pair_id": pid,
                ...
                "proposal_count": 1,
                "independent_graph_proposal_count": 1,
                "proposals": [edge.get("proposal", "deterministic_sentence_window")],
                "reason": str(row.get("reason") or ""),
                "evidence_sentence_ids": row.get("evidence_sentence_ids") or [],
                "score": confidence + (0.02 if edge.get("cue_count", 0) > 0 else 0.0),
            })

        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_decisions_v110.json"), decision_audit)
        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_survivors_v110.json"), accepted)

        best: dict[tuple[str, str, str], dict[str, Any]] = {}
        for edge in accepted:
            k = (edge["source_key"], edge["label"], edge["target_key"])
            if k not in best or edge["score"] > best[k]["score"]:
                best[k] = edge

        candidate_edges = sorted(best.values(), key=lambda e: (-e["score"], e["source_key"], e["target_key"], e["label"]))
        final_edges: list[dict[str, Any]] = []
        handled_pairs: set[tuple[str, str]] = set()
        by_directed_pair: dict[tuple[str, str], list[dict[str, Any]]] = {}
        for edge in candidate_edges:
            by_directed_pair.setdefault((edge["source_key"], edge["target_key"]), []).append(edge)

        for (source_key, target_key), rows in sorted(by_directed_pair.items()):
            pair_key = tuple(sorted((source_key, target_key)))
            if pair_key in handled_pairs:
                continue
            reverse_rows = by_directed_pair.get((target_key, source_key), [])
            here = max(rows, key=lambda e: e["score"])
            if reverse_rows:
                there = max(reverse_rows, key=lambda e: e["score"])
                if abs(here["score"] - there["score"]) >= direction_margin:
                    final_edges.append(here if here["score"] > there["score"] else there)
                else:
                    final_edges.extend([here, there])
                    self._record({
                        "phase": "direction_conflict", "decision": "KEEP_BOTH_NEAR_TIE_FOR_RECALL",
                        "a_to_b": here, "b_to_a": there,
                    })
            else:
                final_edges.append(here)
            handled_pairs.add(pair_key)

        final_edges = sorted(final_edges, key=lambda e: (-e["score"], e["source_key"], e["target_key"]))[:max_final]

        # v1.10 precision refinement: preserve the complete v1.8 result as an audit,
        # then permit ONLY high-confidence removals. The reviewer cannot add/relabel/reverse.
        pre_review_edges: list[dict[str, Any]] = []
        for i, edge in enumerate(final_edges):
            pre_review_edges.append({**edge, "edge_id": f"G{i:04d}"})
        v1.write_json(self.decision_log_path.with_name("layer02_maven_pre_review_edges_v110.json"), pre_review_edges)

        review_decisions: list[dict[str, Any]] = []
        remove_requests: list[dict[str, Any]] = []
        allowed_remove_reasons = {
            "CHRONOLOGY_ONLY", "TOPIC_ONLY", "SAME_EVENT_OR_COREFERENCE", "NO_DEPENDENCY_SUPPORT"
        }

        if graph_review_enabled and pre_review_edges:
            graph_summary = [{
                "edge_id": e["edge_id"],
                "source": str(e.get("source_key") or "").rsplit("::", 1)[-1],
                "relation": e.get("label"),
                "target": str(e.get("target_key") or "").rsplit("::", 1)[-1],
            } for e in pre_review_edges]
            review_batches = [
                pre_review_edges[i:i + graph_review_batch_size]
                for i in range(0, len(pre_review_edges), graph_review_batch_size)
            ]

            def run_graph_review(idx: int, batch: list[dict[str, Any]]) -> list[dict[str, Any]]:
                compact_batch = [{
                    "edge_id": e["edge_id"],
                    "source_trigger": str(e.get("source_key") or "").rsplit("::", 1)[-1],
                    "target_trigger": str(e.get("target_key") or "").rsplit("::", 1)[-1],
                    "relation": e.get("label"),
                    "source_positions": v1._maven_cluster_positions(str(e.get("source_key") or "")),
                    "target_positions": v1._maven_cluster_positions(str(e.get("target_key") or "")),
                    "original_confidence": e.get("confidence"),
                    "original_support_tier": e.get("support_tier"),
                    "original_reason": e.get("reason"),
                    "evidence_sentence_ids": e.get("evidence_sentence_ids") or [],
                } for e in batch]
                messages = [
                    {"role": "system", "content": """
You are the FINAL CONSERVATIVE FALSE-POSITIVE REVIEWER for a MAVEN-ERE relation graph.
The upstream v1.8 classifier has already selected every supplied relation.

DEFAULT = KEEP.
You may ONLY request REMOVE for an edge that is clearly unsupported by the document.
You CANNOT add a relation, change its label, reverse its direction, or propose a replacement.
If uncertain, KEEP by omitting the edge from your response.

MAVEN semantics:
- PRECONDITION: the target genuinely depends on the source as a prerequisite/enabling/initiating/supporting condition. This may be implicit in a plan, process, purpose, or narrative structure; explicit counterfactual wording is NOT required.
- CAUSE: the source more directly brings about / triggers / produces the target.

Do NOT remove merely because:
- the relation is implicit, long-range, goal-related, or part of a larger process;
- one source has many targets (hub structure is valid);
- textual mention order differs from semantic direction;
- evidence is distributed across multiple sentences.

REMOVE is allowed only for one of these reason types:
CHRONOLOGY_ONLY, TOPIC_ONLY, SAME_EVENT_OR_COREFERENCE, NO_DEPENDENCY_SUPPORT.
Use remove_confidence >= 0.86 only when the false-positive diagnosis is very clear.
Return ONLY removal requests; omitted edges are KEEP.
JSON only:
{"remove":[{"edge_id":"G0001","reason_type":"CHRONOLOGY_ONLY|TOPIC_ONLY|SAME_EVENT_OR_COREFERENCE|NO_DEPENDENCY_SUPPORT","remove_confidence":0.91,"reason":"brief source-grounded reason"}]}
""".strip()},
                    {"role": "user", "content": (
                        f"Document:\n{v1._json_block(rows_doc, 52000)}\n\n"
                        f"Whole proposed graph summary (context only):\n{v1._json_block(graph_summary, 24000)}\n\n"
                        f"Edges in THIS review batch:\n{v1._json_block(compact_batch, 30000)}"
                    )},
                ]
                parsed = self._chat(state, messages, f"conservative_graph_prune_v110_{idx:03d}")
                rows = parsed.get("remove", []) if isinstance(parsed, dict) else []
                return rows if isinstance(rows, list) else []

            with ThreadPoolExecutor(max_workers=min(self.max_concurrency, len(review_batches))) as ex:
                futures = {ex.submit(run_graph_review, i, batch): i for i, batch in enumerate(review_batches)}
                for fut in as_completed(futures):
                    try:
                        rows = fut.result()
                    except Exception as exc:
                        self._record({"phase": "graph_review", "batch": futures[fut], "status": "error", "error": str(exc)})
                        continue
                    for row in rows:
                        if not isinstance(row, dict) or not row.get("edge_id"):
                            continue
                        reason_type = str(row.get("reason_type") or "").upper()
                        try:
                            conf = float(row.get("remove_confidence", 0.0) or 0.0)
                        except Exception:
                            conf = 0.0
                        normalized = {
                            "edge_id": str(row.get("edge_id")),
                            "reason_type": reason_type,
                            "remove_confidence": conf,
                            "reason": str(row.get("reason") or ""),
                        }
                        review_decisions.append(normalized)
                        if reason_type in allowed_remove_reasons and conf >= graph_review_remove_min_confidence:
                            remove_requests.append(normalized)

        # Deterministic safety cap: even a pathological reviewer cannot erase more than
        # the configured fraction. Highest-confidence removal requests are applied first.
        unique_remove: dict[str, dict[str, Any]] = {}
        valid_ids = {e["edge_id"] for e in pre_review_edges}
        for row in remove_requests:
            eid = row["edge_id"]
            if eid not in valid_ids:
                continue
            if eid not in unique_remove or row["remove_confidence"] > unique_remove[eid]["remove_confidence"]:
                unique_remove[eid] = row
        max_remove = int(len(pre_review_edges) * graph_review_max_remove_fraction)
        applied_removals = sorted(
            unique_remove.values(), key=lambda r: (-r["remove_confidence"], r["edge_id"])
        )[:max_remove]
        remove_ids = {r["edge_id"] for r in applied_removals}
        final_edges = [e for e in pre_review_edges if e["edge_id"] not in remove_ids]

        v1.write_json(self.decision_log_path.with_name("layer02_maven_graph_review_decisions_v110.json"), review_decisions)
        v1.write_json(self.decision_log_path.with_name("layer02_maven_graph_review_removed_v110.json"), applied_removals)
        v1.write_json(self.decision_log_path.with_name("layer02_maven_final_edges_v110.json"), final_edges)

        for i, edge in enumerate(final_edges):
            enriched.append(v1._relation_enriched(
                expr_id=f"expr_mr110_{i:04d}", source=edge["source_key"], relation_id=edge["label"], target=edge["target_key"],
                metadata=self.catalog[edge["label"]], state=state,
                decision_payload={**edge, "decision": edge["label"], "pipeline": "v1.10_v18_plus_conservative_graph_prune"},
            ))

        counts = {
            "event_mentions": len(keys),
            "relation_anchor_groups": len(anchors),
            "coreference_merging": False,
            "proposal_edges_raw": len(raw_candidates),
            "directed_candidates_after_dedup_cap": len(directed),
            "verifier_survivors": len(accepted),
            "pre_review_relations": len(pre_review_edges),
            "graph_review_enabled": graph_review_enabled,
            "graph_review_remove_requests": len(remove_requests),
            "graph_review_removed": len(applied_removals),
            "graph_review_remove_min_confidence": graph_review_remove_min_confidence,
            "graph_review_max_remove_fraction": graph_review_max_remove_fraction,
            "final_relations": len(final_edges),
            "multi_proposal_min_confidence": multi_min,
            "single_proposal_min_confidence": single_min,
            "max_final_relations": max_final,
            "candidate_generation": "deterministic_all_ordered_local_pairs_v18_reused_in_v110",
            "precision_refinement": "conservative_document_graph_prune_remove_only",
            "sentence_window": sentence_window,
            "preclassification_candidate_cap": None,
            "semantic_direction_filter_before_llm": False,
            "gold_used": False,
        }
        v1.write_json(self.decision_log_path.with_name("layer02_maven_stage_counts_v110.json"), counts)
        return self._finish(state, enriched)

def _maven_stage_posthoc_v110(gold_record: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    """Post-L12 stage diagnostics. Gold is consulted only here, never by Layer 2."""
    gold_rel = v1._gold_relation_set(gold_record, {"CAUSE", "PRECONDITION"})
    gold_pairs = {(s, t) for s, _, t in gold_rel}

    def edge_projection(rows: list[dict[str, Any]], *, labeled: bool) -> dict[str, Any]:
        projected_pairs: set[tuple[str, str]] = set()
        projected_labeled: set[tuple[str, str, str]] = set()
        mapped_rows = 0
        for row in rows:
            ...
            if not sk or not tk:
                continue
            sid = v1._map_maven_endpoint(sk, gold_record)
            tid = v1._map_maven_endpoint(tk, gold_record)
            if not sid or not tid:
                continue
            mapped_rows += 1
            projected_pairs.add((sid, tid))
            if labeled:
                lab = str(row.get("label") or "").upper()
                if lab in {"CAUSE", "PRECONDITION"}:
                    projected_labeled.add((sid, lab, tid))
        pair_hits = projected_pairs & gold_pairs
        labeled_hits = projected_labeled & gold_rel if labeled else set()
        out = {
            "rows": len(rows), "mapped_rows": mapped_rows,
            "projected_directed_pairs": len(projected_pairs),
        ...
        if labeled:
            out.update({
                "projected_labeled_relations": len(projected_labeled),
                "gold_labeled_relations": len(gold_rel),
                "labeled_tp": len(labeled_hits),
                "labeled_recall": len(labeled_hits)/len(gold_rel) if gold_rel else 0.0,
            })
        return out

    files = {
        "directed_candidates": ("layer02_maven_directed_candidates_v110.json", False),
        "verifier_survivors": ("layer02_maven_verifier_survivors_v110.json", True),
        "pre_review_edges": ("layer02_maven_pre_review_edges_v110.json", True),
        "final_edges": ("layer02_maven_final_edges_v110.json", True),
    }
    stages: dict[str, Any] = {}
    for name, (filename, labeled) in files.items():
        path = run_dir / "run_logs" / filename
        if path.exists():
            try:
                rows = v1.read_json(path)
                if isinstance(rows, list):
                    stages[name] = edge_projection(rows, labeled=labeled)
            except Exception as exc:
    ...
    final_path = run_dir / "run_logs" / "layer02_maven_final_edges_v110.json"
    final_projected: set[tuple[str, str, str]] = set()
        ...
        try:
            ...
            for row in rows if isinstance(rows, list) else []:
                ...
                if sid and tid and lab in {"CAUSE", "PRECONDITION"}:
                    final_projected.add((sid, lab, tid))
        except Exception:
            pass
    stages["missing_gold_relations_after_final"] = [
        {"source_gold_id": s, "source_triggers": triggers(s), "label": r,
         "target_gold_id": t, "target_triggers": triggers(t)}
        for s, r, t in sorted(gold_rel - final_projected)
    ]
    ...
    *, dataset_key: str, backends: dict[str, Any], rag_adapter: Any,
    ...
    if dataset_key == "fincausal":
        ...
        pipeline.layers[2] = FinCausalLayer2V13(
            ...
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v110.json",
        ...
        pipeline.layers[2] = MavenLayer2V110(
            backends["layer02"], dataset_key=dataset_key,
            relation_catalog_path=relation_catalog_path,
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v110.json",
...
def run_native_pipeline_record(
    *, dataset_key: str, project_root: str | Path, input_jsonl: str | Path, ontology_path: str | Path,
    ...
    backends = {
        "layer01": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer01_v110", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer01_linguistic_expression_extraction"), fallback_max_tokens=max_tokens, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer02": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer02_v110", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer02_candidate_enrichment"), fallback_max_tokens=4096, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer04": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer04_v110", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer04_candidate_relation_extraction"), fallback_max_tokens=384, fallback_timeout=60, reasoning_effort=reasoning_effort),
        "other": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_other_v110", model_host=host, api_key=api_key, cfg={}, fallback_max_tokens=768, fallback_timeout=90, reasoning_effort=reasoning_effort),
    }
...
def evaluate_state(dataset_key: str, state: PipelineState, gold_record: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        stage_path = run_dir / "run_logs" / "layer02_maven_stage_counts_v110.json"
        if stage_path.exists():
            result["maven_relation_stage_diagnostics"] = v1.read_json(stage_path)
        result["maven_relation_stage_gold_diagnostics"] = _maven_stage_posthoc_v110(gold_record, run_dir)
        v1.write_json(run_dir / "posthoc_maven_relation_stage_metrics_v16.json", result["maven_relation_stage_gold_diagnostics"])
    except Exception as exc:
...
def offline_self_test() -> dict[str, Any]:
    ...
    return {
        **base,
        ...
        "maven_ere": "v1.8 all-ordered radius-4 pair adjudication + conservative remove-only document graph review; frozen v1.4 Layer 1; post-L12 stage diagnostics",
