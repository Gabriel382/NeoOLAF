...
"""Unified4 v1.6 experiment-side adapters.
...
* MAVEN-ERE: v1.4 solved event recall; v1.6 keeps the solved v1.4 event inventory and recalibrates PRECONDITION-aware relation graph selection.
...
class MavenLayer1FrozenV15(MavenLayer1V14):
    ...
    def _reuse_path(self) -> Path:
        ...
        for marker in ("unified4_v1_6", "unified4_v1_5"):
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        if prior.exists():
            try:
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.6 reused frozen v1.4 source inventory: "
                ...
                state.log(
                    f"[{self.name}] MAVEN v1.6 could not reuse v1.4 inventory; "
...
class MavenLayer2V16(v1._TaskLayer2):
    """MAVEN v1.6 relation-only correction.

    The v1.4 Layer-1 event inventory is frozen. v1.5 proved that a skeptic-style
    verifier can over-prune: all gold event clusters were reachable but every true
    relation disappeared. v1.6 therefore keeps singleton mention anchors, uses three
    complementary high-recall whole-document graph proposals, and performs one
    PRECONDITION-aware directed adjudication stage. No gold is available here.
    """
    ...
    def _run(self, state: PipelineState) -> PipelineState:
        ...
        proposal_max = max(24, int(cfg.get("graph_proposal_max_edges", 80)))
        max_candidates = max(40, int(cfg.get("max_directed_candidates", 128)))
        multi_min = float(cfg.get("multi_proposal_min_confidence", 0.50))
        single_min = float(cfg.get("single_proposal_min_confidence", 0.58))
        max_final = max(16, int(cfg.get("max_final_relations", 48)))
        direction_margin = float(cfg.get("opposite_direction_confidence_margin", 0.06))
        rows_doc = v1._safe_token_rows(state)

        # Keep every v1.4 singleton mention distinct. No relation-time coreference merge.
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_anchor_inventory_v16.json"), anchors)

        proposal_inventory = [
            {"anchor_id": a["anchor_id"], "trigger": a["trigger"], "positions": a["positions"]}
            for a in anchors
        ]

        proposal_specs = [
            ("precondition_dependency", r'''
You are NeoOLAF MAVEN-ERE high-recall PRECONDITION graph proposal.
Return directed SOURCE->TARGET candidates when the DOCUMENT presents SOURCE as a
prerequisite, enabling condition, initiating step, supporting action, required process,
or structural dependency for TARGET. MAVEN PRECONDITION is broader than direct
physical causation: the dependency may be strongly implicit in a plan/process/event
structure and need not use explicit counterfactual wording. One SOURCE may legitimately
support multiple TARGET events. Include long-range document-supported dependencies.
Reject only obvious chronology, same-topic association, and same-event restatement.
This is candidate retrieval, not final classification; favor recall while staying grounded.
Use supplied anchor IDs only. Return at most {cap} edges.
JSON only: {"edges":[{"source":"M001","target":"M008","label_hint":"PRECONDITION","support":"...","evidence_sentence_ids":[1,5]}]}.
'''),
            ("process_goal_structure", r'''
You are NeoOLAF MAVEN-ERE event-structure graph proposal. Identify directed dependencies
created by initiation/completion, goal/purpose, enabling/preventing, decision/action,
participation/outcome, process/result, and plan/execution structure. A goal or purpose
may yield PRECONDITION candidates when the document frames one event as enabling or
necessary for another; do not require direct causation. Include nonlocal dependencies.
Do not propose mere co-occurrence, lexical similarity, or identical-event restatements.
This pass favors recall. Use supplied anchor IDs only. Return at most {cap} edges.
JSON only: {"edges":[{"source":"M001","target":"M008","label_hint":"PRECONDITION|CAUSE","support":"...","evidence_sentence_ids":[1,5]}]}.
'''),
            ("direct_cause_result", r'''
You are NeoOLAF MAVEN-ERE direct CAUSE/result graph proposal. Return directed candidates
where SOURCE brings about, triggers, produces, leads to, or strongly results in TARGET.
Also include a PRECONDITION hint instead when the text supports necessity/enabling but
not direct production. Direction is semantic, never textual order. Reject chronology,
shared topic, and coreference. Use supplied anchor IDs only. Return at most {cap} edges.
JSON only: {"edges":[{"source":"M001","target":"M008","label_hint":"CAUSE|PRECONDITION","support":"...","evidence_sentence_ids":[1,5]}]}.
'''),
        ]
        ...
        def run_proposal(name: str, prompt: str) -> tuple[str, list[dict[str, Any]]]:
            messages = [
                ...
                {"role": "user", "content": (
                    ...
                    f"Event mentions (keep mention IDs distinct):\n{v1._json_block(proposal_inventory, 43000)}"
                )},
            ]
            parsed = self._chat(state, messages, f"graph_proposal_{name}_v16")
            edges = parsed.get("edges", []) if isinstance(parsed, dict) else []
            return name, edges if isinstance(edges, list) else []

        with ThreadPoolExecutor(max_workers=min(3, max(1, self.max_concurrency))) as ex:
            ...
            for fut in as_completed(futures):
                ...
                for edge in edges:
                    ...
                    if source not in anchor_by_id or target not in anchor_by_id or source == target:
                        continue
                    hint = str(edge.get("label_hint") or "").upper()
                    if hint not in {"CAUSE", "PRECONDITION"}:
                        hint = ""
        ...
        # Deterministic source-cue rescue: candidate generation only, never a positive label.
        ...
        for a, b in combinations(anchors, 2):
            shared = set(a["sentence_ids"]) & set(b["sentence_ids"])
            cue_sids = [sid for sid in shared if _MAVEN_CAUSAL_CUE_RE.search(sentences.get(sid, ""))]
            if not cue_sids:
                continue
            for source, target in ((a["anchor_id"], b["anchor_id"]), (b["anchor_id"], a["anchor_id"])):
                cue_rows.append({
                    "source": source,
                    "target": target,
                    "proposal": "source_cue_rescue",
                    "label_hint": "",
                    "support": "same source sentence contains a causal/purpose/temporal dependency cue",
                    "evidence_sentence_ids": sorted(cue_sids),
                })
        cue_cap = max(12, int(cfg.get("source_cue_candidate_cap", 36)))
        proposal_rows.extend(cue_rows[:cue_cap])
        v1.write_json(self.decision_log_path.with_name("layer02_maven_proposals_raw_v16.json"), proposal_rows)
        ...
        for row in proposal_rows:
            ...
            item = edge_map.setdefault(key, {
                ...
                "proposals": [], "label_hints": [], "supports": [], "evidence_sentence_ids": [],
            })
            if row["proposal"] not in item["proposals"]:
                item["proposals"].append(row["proposal"])
            if row.get("label_hint") and row["label_hint"] not in item["label_hints"]:
                item["label_hints"].append(row["label_hint"])
            if row.get("support") and row["support"] not in item["supports"]:
                item["supports"].append(row["support"])
            for sid in row.get("evidence_sentence_ids") or []:
                try:
                    sid_i = int(sid)
                except Exception:
                    continue
                if sid_i not in item["evidence_sentence_ids"]:
                    item["evidence_sentence_ids"].append(sid_i)

        directed = list(edge_map.values())
        for edge in directed:
            graph_sources = [p for p in edge["proposals"] if p != "source_cue_rescue"]
            edge["independent_graph_proposal_count"] = len(graph_sources)
            edge["proposal_count"] = len(edge["proposals"])
            sa, ta = anchor_by_id[edge["source"]], anchor_by_id[edge["target"]]
            edge["source_key"] = sa["key"]
            edge["target_key"] = ta["key"]
            edge["source_trigger"] = sa["trigger"]
            edge["target_trigger"] = ta["trigger"]
            edge["source_positions"] = sa["positions"]
            edge["target_positions"] = ta["positions"]
            dists = [abs(x[0]-y[0]) for x in sa["positions"] for y in ta["positions"]]
            edge["min_sentence_distance"] = min(dists) if dists else 999

        directed.sort(key=lambda e: (
            -e["independent_graph_proposal_count"], -e["proposal_count"],
            e["min_sentence_distance"], e["source"], e["target"],
        ))
        directed = directed[:max_candidates]
        for i, edge in enumerate(directed):
            edge["pair_id"] = f"D{i:04d}"
        v1.write_json(self.decision_log_path.with_name("layer02_maven_directed_candidates_v16.json"), directed)
        ...
        def run_adjudicate(idx: int, batch: list[dict[str, Any]]) -> list[dict[str, Any]]:
            compact = [{
                ...
                "proposal_sources": e["proposals"],
                "proposal_label_hints": e["label_hints"],
                "proposal_support": e["supports"],
            } for e in batch]
            messages = [
                {"role": "system", "content": r'''
You are NeoOLAF MAVEN-ERE directed relation adjudication. For every proposed SOURCE->TARGET,
choose exactly one of KEEP_PRECONDITION, KEEP_CAUSE, REVERSE_PRECONDITION,
REVERSE_CAUSE, or REJECT.

Use the MAVEN relation convention:
- PRECONDITION: TARGET depends on SOURCE as a prerequisite/enabling/initiating/supporting
  condition in the event structure. The document may express this directly OR strongly
  imply it through purpose, plan, process, initiation/completion, enabling, participation,
  or structural dependency. Do NOT demand an explicit phrase like "would not happen without".
  One SOURCE may validly be a precondition for several TARGET events.
- CAUSE: SOURCE more directly brings about, produces, triggers, or makes TARGET effectively
  follow as a result. Use CAUSE more narrowly than PRECONDITION.

Ground every positive in this document. Reject mere chronology, co-occurrence, same topic,
lexical association, and two mentions of the same real-world event. However, do NOT reject
an otherwise supported PRECONDITION merely because it is long-range, implicit, goal-related,
or part of a larger event plan. Direction is semantic, never textual mention order.

Calibrate confidence: 0.50-0.65 = plausible but supported; 0.66-0.80 = clear; >0.80 = very clear.
Return support_tier DIRECT, STRONG_IMPLICIT, WEAK, or NONE. A positive may use DIRECT or
STRONG_IMPLICIT support. JSON only:
{"decisions":[{"pair_id":"D0000","decision":"KEEP_PRECONDITION|KEEP_CAUSE|REVERSE_PRECONDITION|REVERSE_CAUSE|REJECT","support_tier":"DIRECT|STRONG_IMPLICIT|WEAK|NONE","evidence_sentence_ids":[1,5],"confidence":0.67,"reason":"..."}]}
'''.strip()},
                {"role": "user", "content": (
                    f"Document:\n{v1._json_block(rows_doc, 52000)}\n\n"
            ...
            parsed = self._chat(state, messages, f"precondition_aware_adjudicate_v16_{idx:03d}")
        ...
        for pid, edge in by_pid.items():
            ...
            try:
                ...
                confidence = 0.0
            graph_count = int(edge.get("independent_graph_proposal_count", 0) or 0)
            threshold = multi_min if graph_count >= 2 else single_min
            positive = decision in {
                "KEEP_PRECONDITION", "KEEP_CAUSE", "REVERSE_PRECONDITION", "REVERSE_CAUSE"
            }
            ...
            self._record({"phase": "precondition_aware_adjudicate", **audit})
            if not accept:
                continue
            ...
            accepted.append({
                ...
                "proposal_count": edge.get("proposal_count", 0),
                "independent_graph_proposal_count": graph_count,
                "proposals": edge.get("proposals", []),
                "reason": str(row.get("reason") or ""),
                "evidence_sentence_ids": row.get("evidence_sentence_ids") or [],
                "score": confidence + 0.03 * min(graph_count, 3),
            })

        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_decisions_v16.json"), decision_audit)
        v1.write_json(self.decision_log_path.with_name("layer02_maven_verifier_survivors_v16.json"), accepted)
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_final_edges_v16.json"), final_edges)

        for i, edge in enumerate(final_edges):
            enriched.append(v1._relation_enriched(
                expr_id=f"expr_mr16_{i:04d}",
                source=edge["source_key"], relation_id=edge["label"], target=edge["target_key"],
                metadata=self.catalog[edge["label"]], state=state,
                decision_payload={**edge, "decision": edge["label"], "pipeline": "v1.6_precondition_aware_graph"},
            ))
        ...
        v1.write_json(self.decision_log_path.with_name("layer02_maven_stage_counts_v16.json"), counts)
        return self._finish(state, enriched)


def _maven_stage_posthoc_v16(gold_record: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    files = {
        "directed_candidates": ("layer02_maven_directed_candidates_v16.json", False),
        "verifier_survivors": ("layer02_maven_verifier_survivors_v16.json", True),
        "final_edges": ("layer02_maven_final_edges_v16.json", True),
    }
    stages: dict[str, Any] = {}
    ...
    final_path = run_dir / "run_logs" / "layer02_maven_final_edges_v16.json"
...
def build_pipeline(
    *, dataset_key: str, backends: dict[str, Any], rag_adapter: Any,
    ...
    # only the experiment-side L1/L2 adapters; v1.6 changes MAVEN Layer 2 only.
    ...
    if dataset_key == "fincausal":
        ...
        pipeline.layers[2] = FinCausalLayer2V13(
            ...
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v16.json",
        ...
        pipeline.layers[2] = MavenLayer2V16(
            backends["layer02"], dataset_key=dataset_key,
            relation_catalog_path=relation_catalog_path,
            decision_log_path=run_dir / "run_logs/layer02_relation_decisions_v16.json",
...
def run_native_pipeline_record(
    *, dataset_key: str, project_root: str | Path, input_jsonl: str | Path, ontology_path: str | Path,
    ...
    backends = {
        "layer01": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer01_v16", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer01_linguistic_expression_extraction"), fallback_max_tokens=max_tokens, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer02": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer02_v16", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer02_candidate_enrichment"), fallback_max_tokens=4096, fallback_timeout=request_timeout, reasoning_effort=reasoning_effort),
        "layer04": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_layer04_v16", model_host=host, api_key=api_key, cfg=v1._layer_cfg(profile_dict, "layer04_candidate_relation_extraction"), fallback_max_tokens=384, fallback_timeout=60, reasoning_effort=reasoning_effort),
        "other": v1._make_backend(logger=logger, layer_tag=f"{dataset_key}_other_v16", model_host=host, api_key=api_key, cfg={}, fallback_max_tokens=768, fallback_timeout=90, reasoning_effort=reasoning_effort),
    }
...
def evaluate_state(dataset_key: str, state: PipelineState, gold_record: dict[str, Any]) -> dict[str, Any]:
    """v1.6 preserves frozen v1.4 Layer-1 diagnostics and adds post-L12 per-stage relation recall diagnostics."""
    ...
    try:
        ...
        stage_path = run_dir / "run_logs" / "layer02_maven_stage_counts_v16.json"
        if stage_path.exists():
            result["maven_relation_stage_diagnostics"] = v1.read_json(stage_path)
        result["maven_relation_stage_gold_diagnostics"] = _maven_stage_posthoc_v16(gold_record, run_dir)
...
def offline_self_test() -> dict[str, Any]:
    ...
    return {
        **base,
        ...
        "maven_ere": "frozen/reused v1.4 event inventory + singleton anchors + three high-recall graph proposals + PRECONDITION-aware adjudication + posthoc stage recall diagnostics",
