...
from olaf.pipeline.pipeline_component.concept_relation_hierarchy.subsumption_hierarchisation import (
    ...
    c1 = Concept(
        ...
        linguistic_realisations={
            LinguisticRealisation(
                label="test", corpus_occurrences={corpus[0][1], corpus[2][5]}
            ),
            LinguisticRealisation(label="tests", corpus_occurrences={corpus[1][3]}),
        },
    )
    return c1
...
def c2(corpus):
    c2 = Concept(
        label="sentence",
        linguistic_realisations={
            LinguisticRealisation(label="sentence", corpus_occurrences={corpus[0][2]}),
            LinguisticRealisation(label="sentences", corpus_occurrences={corpus[2][6]}),
        },
    )
    return c2
...
def c3(corpus):
    c3 = Concept(
        ...
        linguistic_realisations={
            LinguisticRealisation(
                label="first test", corpus_occurrences={corpus[0][:1], corpus[2][4:5]}
            )
        },
    )
    return c3
...
@pytest.fixture(scope="session")
def pipeline(en_sm_spacy_model, corpus, concepts):
